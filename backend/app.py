...
@login_required
def serve_my_reviews():
    """Serve the My Reviews page for users to manage their reviews"""
    return render_template('my_reviews.html')

@app.route('/test_reviews')
@login_required
def serve_test_reviews():
    """Serve the test reviews page for debugging"""
    return render_template('test_reviews.html')

@app.route('/qr_scanner')
@login_required
def serve_qr_scanner():
    """Serve the QR scanner page for scanning event QR codes"""
...
@cache.cached(timeout=300, query_string=True)  # PERFORMANCE: Cache for 5 minutes (with query string)
def api_get_all_events():
    try:
        events_data = get_events_cached()  # PERFORMANCE: Use cached data
        
        # Check if admin wants only their events
        my_events_only = request.args.get('my_events_only', 'false').lower() == 'true'
        
        if my_events_only and session.get('admin_logged_in'):
            # Filter events to show only those created by this admin
            admin_id = session.get('admin_id')
            events_data = [event for event in events_data if event.get('created_by') == admin_id]
...
@app.route('/api/delete-downloads', methods=['POST'])
@login_required
def delete_downloads():
    """Delete selected downloads from user's history"""
    try:
        user_id = session.get('user_id')
        ...
        if not download_ids:
            return jsonify({
                "success": False,
                "error": "No download IDs provided"
            }), 400
        ...
        # Delete downloads - only allow users to delete their own downloads
        placeholders = ','.join(['%s'] * len(download_ids))
        query = f"""DELETE FROM downloads 
                    WHERE id IN ({placeholders}) 
                    AND user_id = %s"""
        
        cursor.execute(query, (*download_ids, user_id))
        conn.commit()
        ...
        return jsonify({
            ...
            "message": f"Successfully deleted {deleted_count} download(s)"
        })
...
def get_reviews():
    """Get all reviews or top reviews (general reviews only by default)"""
    try:
        ...
        # By default, show only general reviews (is_general_review = 1)
        if top_only:
            # Get top 3 general reviews (highest rated, most recent)
            query = """SELECT r.*, u.email as user_email 
                      FROM reviews r
                      JOIN users u ON r.user_id = u.id
                      WHERE r.is_general_review = 1
                      ORDER BY r.rating DESC, r.created_at DESC
                      LIMIT 3"""
        elif limit:
            query = f"""SELECT r.*, u.email as user_email 
                       FROM reviews r
                       JOIN users u ON r.user_id = u.id
                       WHERE r.is_general_review = 1
...
@login_required
def create_review():
    """Create a new review (general or event-specific with QR verification)"""
    try:
        user_id = session.get('user_id')
        ...
        event_id = data.get('event_id')  # Optional: if provided, it's an event review
        is_general = data.get('is_general_review', event_id is None)  # True if no event_id
        ...
        # Handle event-specific review
        if event_id and not is_general:
            # Verify user has scanned QR code for this event
            cursor.execute(
                "SELECT id FROM event_attendees WHERE user_id = %s AND event_id = %s",
                (user_id, event_id)
            )
            ...
            if not attendance:
                ...
                return jsonify({
                    "success": False,
                    "error": "You need a valid event QR code to write a review for this event."
                }), 403
            
            # Check if user already reviewed this event
            cursor.execute(
                "SELECT id FROM reviews WHERE user_id = %s AND event_id = %s AND is_general_review = 0",
                (user_id, event_id)
            )
            ...
            if existing_review:
                ...
                cursor.execute(
                    """UPDATE reviews 
                       ...
                       WHERE user_id = %s AND event_id = %s AND is_general_review = 0""",
                    (rating, review_text, user_id, event_id)
                )
                message = "Event review updated successfully"
            else:
                # Insert new event review
                cursor.execute(
                    """INSERT INTO reviews (user_id, user_name, rating, review_text, event_id, is_general_review)
                       VALUES (%s, %s, %s, %s, %s, 0)""",
                    (user_id, user_name, rating, review_text, event_id)
                )
                message = "Event review created successfully"
        
        # Handle general website review
        else:
            # Check if user already has a general review
            cursor.execute(
                "SELECT id FROM reviews WHERE user_id = %s AND is_general_review = 1",
                (user_id,)
            )
            existing_review = cursor.fetchone()
            
            if existing_review:
                # Update existing general review
                cursor.execute(
                    """UPDATE reviews 
                       SET rating = %s, review_text = %s, updated_at = NOW()
                       WHERE user_id = %s AND is_general_review = 1""",
                    (rating, review_text, user_id)
                )
                message = "General review updated successfully"
            else:
                # Insert new general review
                cursor.execute(
                    """INSERT INTO reviews (user_id, user_name, rating, review_text, event_id, is_general_review)
...
@app.route('/api/check-event-attendance', methods=['GET'])
@login_required
def check_event_attendance():
    """Check if user has scanned QR code for an event"""
    try:
        user_id = session.get('user_id')
        ...
        return jsonify({
            "success": True,
            "has_scanned": result is not None
        })
...
@app.route('/api/scan-event-qr', methods=['POST'])
@login_required
def scan_event_qr():
    """Record that user has scanned an event QR code"""
    try:
        user_id = session.get('user_id')
        if not user_id:
            return jsonify({
        ...
        try:
            ...
            conn.commit()
            message = "Event attendance recorded. You can now write a review for this event!"
        except:
            # Already exists
            message = "You have already scanned this event QR code."
        ...
        print(f"Error recording event attendance: {e}")
        return jsonify({
            "success": False,
...
@login_required
def get_user_review():
    """Get all reviews by the current user (general + event-specific)"""
    try:
        user_id = session.get('user_id')
        ...
        # Get all reviews by user (no JOIN needed - events are in JSON file)
        cursor.execute(
            """SELECT r.* 
               FROM reviews r
               WHERE r.user_id = %s
               ORDER BY r.is_general_review DESC, r.created_at DESC""",
            (user_id,)
        )
        reviews = cursor.fetchall()
        
        # Load events data from JSON to get event names
        events_data = get_events_cached()
        events_map = {event['id']: event['name'] for event in events_data}
        
        # Add event names to reviews
        for review in reviews:
            if review.get('event_id') and review['event_id'] in events_map:
                review['event_name'] = events_map[review['event_id']]
            else:
                review['event_name'] = None
        ...
        print(f"Error retrieving user review: {e}")
        return jsonify({
            "success": False,
            "error": "Failed to retrieve review"
        }), 500

@app.route('/api/reviews/event/<event_id>', methods=['GET'])
    ...
    try:
        ...
        cursor.execute(
            """SELECT r.*, u.email as user_email 
               FROM reviews r
               JOIN users u ON r.user_id = u.id
               WHERE r.event_id = %s AND r.is_general_review = 0
               ORDER BY r.created_at DESC""",
            (event_id,)
        )
        reviews = cursor.fetchall()
        ...
        return jsonify({
            "success": False,
            "error": "Failed to retrieve event reviews"
        }), 500
